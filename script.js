...
document.addEventListener("DOMContentLoaded", () => {
  const elementsToAnimate = document.querySelectorAll(
    ".comic-panel, .project-card, .section-header"
  );
...
function openProject(projectId) {
  ...
  alert(
    `Opening project ${projectId}! You can customize this function to show project details.`
  );
...
function showAlert(message, type = "info") {
  ...
  alert.style.cssText = `
        ...
        right: 20px;
        background: ${
          type === "success" ? "var(--primary)" : "var(--destructive)"
        };
        color: white;
        padding: 1rem 1.5rem;
        border-radius: 8px;
